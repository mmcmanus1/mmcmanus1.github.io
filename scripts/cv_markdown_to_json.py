...
def parse_publications(pubs_dir):
    """Parse publications from the _papers directory."""
    publications = []
    
    if not os.path.exists(pubs_dir):
        return publications
    
    for pub_file in sorted(glob.glob(os.path.join(pubs_dir, "*.md"))):
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    cv_json = {
        ...
        "interests": [],
        "references": [],
        "papers": parse_publications(os.path.join(repo_root, "_papers"))
    }
